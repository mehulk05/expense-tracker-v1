  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-5xl mx-auto space-y-8">
       ...
       <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 pb-6 border-b border-slate-200">
        <div>
          <button onClick={() => navigate('/splitwise')} className="text-[10px] text-slate-400 font-bold mb-3 hover:text-blue-600 flex items-center gap-1 uppercase tracking-wider transition-colors">
             <ICONS.ChevronRight className="w-3.5 h-3.5 rotate-180" /> Back to Dashboard
          </button>
          <h1 className="text-lg font-bold text-slate-800 tracking-tight uppercase">{group.name}</h1>
          <p className="text-slate-500 text-[10px] font-bold uppercase tracking-wider mt-1">{expenses.length} expenses • {members.length} members</p>
        </div>
      ...
      <div className="flex gap-2 border-b border-slate-200">
          <button 
            ...
            className={`px-6 py-3 text-[10px] font-bold uppercase tracking-wider transition-all border-b-2 ${activeTab === 'expenses' ? 'border-blue-600 text-blue-600' : 'border-transparent text-slate-400 hover:text-slate-600'}`}
          >
              Expenses
          </button>
          <button 
             onClick={() => setActiveTab('balances')}
             className={`px-6 py-3 text-[10px] font-bold uppercase tracking-wider transition-all border-b-2 ${activeTab === 'balances' ? 'border-blue-600 text-blue-600' : 'border-transparent text-slate-400 hover:text-slate-600'}`}
          >
              Balances
          </button>
          <button 
             onClick={() => setActiveTab('settings')}
             className={`px-6 py-3 text-[10px] font-bold uppercase tracking-wider transition-all border-b-2 ${activeTab === 'settings' ? 'border-blue-600 text-blue-600' : 'border-transparent text-slate-400 hover:text-slate-600'}`}
          >
              Settings
          </button>
      </div>
      ...
      {activeTab === 'expenses' ? (
          <div className="space-y-6">
              <div className="relative">
                  ...
                  <input 
                    ...
                    className="w-full pl-11 pr-4 py-2.5 bg-white border border-slate-200 rounded-lg text-sm font-semibold text-slate-700 outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 transition-colors hover:border-slate-300 shadow-sm"
                  />
              </div>
                           ...
                           <div key={exp.id} className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm hover:shadow-md transition-all flex items-center justify-between group">
                              <div className="flex items-center gap-4">
                                  <div className="w-14 h-14 bg-slate-50 rounded-xl border border-slate-100 flex flex-col items-center justify-center text-slate-500">
                                      <span className="text-[10px] font-bold uppercase text-slate-400 leading-none mb-1">{month}</span>
                                      <span className="text-xl font-bold text-slate-800 leading-tight">{day}</span>
                                      <span className="text-[9px] font-bold text-slate-400 leading-none mt-1">{year}</span>
                                  </div>
                                  <div>
                                      <h3 className="text-base font-bold text-slate-800">{exp.title}</h3>
                                      <div className="flex items-center gap-2 mt-1">
                                          <div className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider ${payer?.id ? 'bg-blue-50 text-blue-600' : 'bg-slate-100 text-slate-500'}`}>
                              ...
                              <div className="text-right">
                                   <div className="text-lg font-bold text-slate-800 mb-1">{formatCurrency(exp.amount)}</div>
                                   <div className="flex justify-end gap-2 mb-2">
                                       <button onClick={() => handleEditExpense(exp)} className="p-1.5 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded transition-colors">
                                           <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
                                       </button>
                                       <button onClick={() => handleDeleteExpense(exp.id)} className="p-1.5 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded transition-colors">
          ...
          <div className="space-y-8">
               ...
               <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm mt-4">
                   <h3 className="text-sm font-bold text-slate-500 uppercase tracking-wide mb-6 px-2">Group Settings</h3>
               ...
               <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                   <div className="flex justify-between items-center mb-6">
                       <h3 className="text-sm font-bold text-slate-500 uppercase tracking-wide">Manage Members</h3>
                       ...
                       {members.map(m => (
                           <div key={m.id} className="flex items-center justify-between p-3 bg-slate-50 rounded-xl border border-slate-100">
                               <div className="flex items-center gap-3">
                                   <img src={m.avatar} className="w-9 h-9 rounded-full bg-white border border-slate-200" />
                                   <div>
                                       <p className="font-bold text-slate-800 text-sm">{m.name}</p>
                                       <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">{m.email}</p>
                                   </div>
                               </div>
                               <button 
                                   onClick={async () => {
               ...
               {/* 3. Danger Zone */}
                <div className="bg-red-50 p-6 rounded-2xl border border-red-100">
                   ...
                   <p className="text-xs text-red-600/70 mb-6 font-medium">Deleting a group is permanent and will remove all associated expenses history.</p>
                                      <button 
                        onClick={async () => {
                        ...
                        className="w-full py-3.5 rounded-xl bg-red-600 text-white font-bold uppercase text-[10px] tracking-widest hover:bg-red-700 transition-all shadow-sm shadow-red-100"
                    >
                        Delete Group
                    </button>
               </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                   {/* Modern Total Spend Card */}
                    <div className="bg-white p-8 rounded-2xl border border-slate-200 shadow-sm relative overflow-hidden group">
                        <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
                            <ICONS.Chart className="w-24 h-24 text-blue-600 rotate-12" />
                        </div>
                        <div className="relative z-10">
                            <p className="text-slate-400 text-[10px] font-bold uppercase tracking-widest mb-2">Total Group Spending</p>
                            <h2 className="text-3xl font-bold tracking-tight text-slate-800 mb-3">
                                {formatCurrency(totalGroupSpend)}
                            </h2>
                            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-lg bg-emerald-50 text-emerald-600 border border-emerald-100">
                                <span className="text-[10px] font-bold uppercase tracking-wider">All time tracked</span>
                            </div>
                        </div>
                    </div>
                   ...
                   {/* Enhanced Spending by Person */}
                    <div className="bg-white p-8 rounded-2xl border border-slate-200 shadow-sm flex flex-col justify-between">
                        <h3 className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-4">Top Spenders</h3>
                       <div className="space-y-4">
                           {(Object.entries(spendingByPerson) as [string, number][]).sort((a,b) => b[1] - a[1]).slice(0, 3).map(([pid, amt]) => {
                                    ...
                                    <div key={pid} className="group">
                                        <div className="flex justify-between items-end mb-2">
                                            <div className="flex items-center gap-2">
                                                <img src={person?.avatar} className="w-6 h-6 rounded-full bg-slate-50 border border-slate-100" />
                                                <span className="text-xs font-bold text-slate-600">{person?.name}</span>
                                            </div>
                                            <span className="text-xs font-bold text-slate-800">{formatCurrency(amt)}</span>
                                        </div>
                  ...
                  {/* Member Balances */}
                   <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                       <div className="p-6 border-b border-slate-100 bg-slate-50/50">
                           <h3 className="text-sm font-bold text-slate-500 uppercase tracking-wide">Net Balances</h3>
                           <p className="text-[10px] text-slate-400 font-bold mt-1 uppercase tracking-wider">Who owes what in total</p>
                       </div>
                              ...
                              return (
                                  <div key={m.id} className="p-4 flex items-center justify-between hover:bg-slate-50 transition-colors">
                                      <div className="flex items-center gap-3">
                                       ...
                                       <div className="text-right">
                                           {isZero ? (
                                               <span className="inline-block px-3 py-1.5 rounded-lg bg-slate-100 text-slate-400 text-[10px] font-bold uppercase tracking-wider">Settled</span>
                                           ) : bal > 0 ? (
                                               <div>
                                                   <p className="text-[10px] uppercase font-bold text-emerald-500 tracking-wider mb-1">Gets back</p>
                                                   <p className="text-sm font-bold text-emerald-600">{formatCurrency(bal)}</p>
                                               </div>
                                           ) : (
                                               <div>
                                                   <p className="text-[10px] uppercase font-bold text-red-500 tracking-wider mb-1">Owes</p>
                                                   <p className="text-sm font-bold text-red-500">{formatCurrency(Math.abs(bal))}</p>
                                               </div>
                                           )}
                                       </div>
                                  </div>
                  ...
                  {/* Settlement Plan */}
                   <div className="bg-slate-50 rounded-2xl border border-slate-200 overflow-hidden flex flex-col">
                        <div className="p-6 border-b border-slate-200 bg-white">
                            <h3 className="text-sm font-bold text-slate-500 uppercase tracking-wide">Settlement Plan</h3>
                            <p className="text-[10px] text-slate-400 font-bold mt-1 uppercase tracking-wider">Recommended way to settle up</p>
                        </div>
                                   ...
                                   const to = members.find(m => m.id === s.to);
                                    return (
                                        <div key={idx} className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex items-center justify-between group hover:shadow-md transition-all">
                                            <div className="flex items-center gap-3">
                                                <div className="relative">
                                                    <img src={from?.avatar} className="w-9 h-9 rounded-full border border-slate-100" />
                                                    <div className="absolute -bottom-1 -right-1 bg-white rounded-full p-0.5 border border-slate-100 shadow-sm">
                                                         <ICONS.ChevronRight className="w-2.5 h-2.5 text-blue-600" />
                                                    </div>
                                                </div>
                                                <div className="flex flex-col">
                                                    <span className="text-xs font-bold text-slate-800">{from?.name}</span>
                                                    <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">pays {to?.name}</span>
                                                </div>
                                            </div>
                                            <div className="bg-blue-50 px-3 py-1.5 rounded-lg border border-blue-100">
                                                 <span className="text-xs font-bold text-blue-700">{formatCurrency(s.amount)}</span>
                                            </div>
                                        </div>
                                    )
                               })
